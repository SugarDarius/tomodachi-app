...
import { type ColumnMapping, CanonicalContactField } from '~/schema'
...
export function ImportColumnsMapper({
  headPreview,
  columnMapping,
  onUpdateColumnMapping,
  onImportContacts,
}: {
  headPreview: PreviewResult
  columnMapping: ColumnMapping
  onUpdateColumnMapping: (columnMapping: ColumnMapping) => void
  onImportContacts: () => void
}) {
  const { fileSizeBytes, sampleRows, fileName, headers } = headPreview

  const allRequiredRolesMapped = checkAllRequiredRolesMapped(
    columnMapping,
    REQUIRED_ROLES
  )
  const mappedRequiredRoles = getMappedRequiredRoles(
    columnMapping,
    REQUIRED_ROLES
  )
  ...
  const handleUpdateColumnMapping = useCallback(
    (
      ...
      let varying = [...columnMapping.varying]
      ...
      if (role === 'varying') {
        varying.push({ value: header, positionIndex })
      } else if (role === 'email') {
        canonical.email = { value: header, positionIndex }
      } else if (role === 'first_name') {
        canonical.first_name = { value: header, positionIndex }
      } else if (role === 'last_name') {
        canonical.last_name = { value: header, positionIndex }
      }

      onUpdateColumnMapping({ canonical, varying })
    },
    [columnMapping, onUpdateColumnMapping]
  )
  ...
  return (
    <div className='flex flex-col gap-2 flex-1 overflow-hidden'>
      <div className='flex items-center justify-between p-4 rounded-lg bg-background border border-border/50 flex-non'>
        ...
        <Button disabled={!allRequiredRolesMapped} onClick={onImportContacts}>
          Import contacts
        </Button>
      </div>
      ...
      <div className='flex flex-col p-4 border-b border-border/50 bg-background gap-2 flex-none'>
        ...
        <div className='flex gap-2'>
          {REQUIRED_ROLES.map((role) => {
            const isMapped = isRequiredRoleMapped(columnMapping, role)
            ...
            {headers.map((header, index) => {
              ...
              return (
                ...
                >
                  ...
                  <Select
                    value={getColumnMapValue(columnMapping, header)}
                    onValueChange={(value) => {
              ...
              <div className='overflow-x-auto'>
                <table className='w-full text-xs'>
                  <thead className='bg-muted sticky top-0'>
                    <tr>
                      {headPreview.headers.map((h, colIdx) => (
                        <th
                          key={`${colIdx}:${h}`}
